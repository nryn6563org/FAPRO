...
import {
  TodayIssueWidget,
  PriceAnalysisWidget,
  SupplyAnalysisWidget,
  ThemeSectorWidget,
  ReportAnalysisWidget,
  DisclosureAnalysisWidget
} from '@/app/components/investment-widgets';
...
export function Widget({ widgetId, onRemove }: WidgetProps) {
  ...
  // Investment Info Widgets
  if (widgetId === 'today-issue-widget') {
    return <TodayIssueWidget onRemove={onRemove} />;
  }

  if (widgetId === 'price-analysis-widget') {
    return <PriceAnalysisWidget onRemove={onRemove} />;
  }

  if (widgetId === 'supply-analysis-widget') {
    return <SupplyAnalysisWidget onRemove={onRemove} />;
  }

  if (widgetId === 'theme-sector-widget') {
    return <ThemeSectorWidget onRemove={onRemove} />;
  }

  if (widgetId === 'report-analysis-widget') {
    return <ReportAnalysisWidget onRemove={onRemove} />;
  }

  if (widgetId === 'disclosure-analysis-widget') {
    return <DisclosureAnalysisWidget onRemove={onRemove} />;
  }

  // AI Recommend Widgets - Placeholder
  if (widgetId === 'today-pick-widget' || widgetId === 'daily-stocks-widget' ||
